...
namespace SOAPMessageBuilder
{
    public partial class Form1 : Form
    {
        ...
        private void checkBinding(string bindingName, string operationName, XmlNamespaceManager nsmgr)
        {
            ...
            var portTypeValue = findNode("bindingPortType", "/wsdl:definitions/wsdl:binding[@name='" + tbBindingName.Text + "']/@type", WSDLContent.DocumentElement, nsmgr).Value;
            ...
            BindingInfo.SOAPBindingTransport = findNode("bindingPortType", "/wsdl:definitions/wsdl:binding[@name='" + tbBindingName.Text + "']/soap:binding/@transport", WSDLContent.DocumentElement, nsmgr).Value;

            //Binding operation node for finding style and use attributes
            var nodes = findNode("binding", "/wsdl:definitions/wsdl:binding[@name='" + tbBindingName.Text + "']/wsdl:operation[@name = '" + tbOperatioName.Text + "']", WSDLContent.DocumentElement, nsmgr);
        ...
        {
            getXMLfromURI(WSDLContent, tbWSDLUrl.Text);

            //WSDL loading for display.
            tbWSDL.Text = WSDLContent.OuterXml;
        }
        ...
        /// <returns></returns>
        private XmlDocument getXMLfromURI(XmlDocument document, string uri)
        {
            // Create a request for the URL.   
            WebRequest request = WebRequest.Create(uri);

            if (chbNeedLogin.Checked)
            {
        ...
        private void btnGenerateSoapMessage_Click(object sender, EventArgs e)
        {
            ...
            if (tbBindingName.Text.EndsWith("12"))
            {
            ...
            checkBinding(tbBindingName.Text, tbOperatioName.Text, nsmgr);

            //Collect Operation specific port type data.
            getTypeOfPortAndOperationMessageReferences(BindingInfo.PortTypeName, tbOperatioName.Text, nsmgr);
            ...
            var trimmedSchema = trimOutSchemaXML(schema, tbOperatioName.Text, out targetNamespace);
            ...
            tbSOAPMessage.Text = buildSOAPEnvelope(soapBodyXML);
        }

        /// <summary>
        /// Trims out execess element definitions from schema XML, that are not releted to particular service operation request type.
        /// </summary>
        /// <param name="schemaXML">Schema XML document.</param>
        /// <param name="nodeNameToKeep">The operation request element type definition node in schema.</param>
        /// <param name="schemaTargetNamespaceUri">Generic XML schema language definition namspace in URI format.</param>
        /// <returns></returns>
